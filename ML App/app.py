...
def show_demo_images():
    demo_images = [
        "demo1.jpg",    # Replace with actual demo image paths
        "demo2.png",  # Replace with actual demo image paths
        "demo3.jpg",  # Replace with actual demo image paths
        "demo4.jpg",  # Replace with actual demo image paths
        "demo5.jpg",  # Replace with actual demo image paths
        "demo6.jpg",  # Replace with actual demo image paths
    ]
    ...
    if len(demo_images) % rows != 0:
        cols_per_row += 1  # If there is a remainder, increase column count for the last row
        ...
        for col in range(cols_per_row):
            ...
            if index < len(demo_images):  # Check if the index is within the demo images list
                ...
                with open(img_path, "rb") as file:
                    ...
                    demo_img = demo_img.resize((250, 250), Image.LANCZOS)  # Resize to fixed size
                    
                    # Set the caption based on the index
                    caption = f'Demo Image {index + 1}: {"With Mask" if index % 2 else "Without Mask"}'
                    cols[col].image(demo_img, caption=caption, use_column_width=False)  # Use column width set to false
